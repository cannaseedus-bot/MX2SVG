...
(function(global) {
  ...
  const MASK_REASON = {
    PHASE_MISMATCH: 'M1_PHASE_MISMATCH',
    ZERO_INFO_GAIN: 'M2_ZERO_INFO_GAIN',
    LANE_BLOCKED: 'M3_LANE_BLOCKED',
    COLLAPSE_LOCKED: 'M5_COLLAPSE_LOCKED',
    BOOTSTRAP_ONLY: 'M6_BOOTSTRAP_ONLY',
    SIGNATURE_WAIT: 'M7_SIGNATURE_WAIT'
  };
  ...
  class ABREngine {
    ...
    step() {
      ...
      // 2. Derive mask
      const { mask: runmask, reasons } = this._deriveMaskWithReasons();
      const maskEvt = this._emitMaskEvent(runmask, reasons);
      events.push(maskEvt);
     ...
     */
    _deriveMask() {
      return this._deriveMaskWithReasons().mask;
    }

    /**
     * Derive mask and mask reasons for replay validation.
     */
    _deriveMaskWithReasons() {
      ...
      const collapseLocked = this.phase === 'act' || this.phase === 'reflect';
      ...
      for (let i = 0; i < 28; i++) {
        ...
        // Mandatory masking rules (single reason)
        if (!domainAllowed) {
          reason = MASK_REASON.PHASE_MISMATCH;
        } else if (!laneAllowed) {
          reason = MASK_REASON.LANE_BLOCKED;
        } else if (collapseLocked && profile.domain !== 'SAFE') {
          reason = MASK_REASON.COLLAPSE_LOCKED;
        } else if (i === 24 && this.tick > 0) {
          reason = MASK_REASON.BOOTSTRAP_ONLY;
        } else if (i === 27 && !this.lastAnswer) {
          reason = MASK_REASON.SIGNATURE_WAIT;
        } else if (entropySaturated) {
          reason = MASK_REASON.ZERO_INFO_GAIN;
        }
    ...
    _emitProofEvent(block, signal) {
      ...
      const proofHash = this._hashHex(this._canon({
        '@type': '⚡',
    ...
    _emitMaskEvent(runmask, reasons) {
      ...
      return {
        ...
        '@policy_hash': this.policyHash,
        '@proof': this._hashHex(this._canon({
          tick: this.tick, phase: this.phase, runmask: u32, reasons
        }))
      };
    }
  ...
  const SCXQ2 = {
    ...
    encode: function(engine) {
      ...
      // FRAME STREAM section (length-prefixed JSON)
      const framePayload = {
        frames: engine.frames,
        lastAnswer: engine.lastAnswer,
        lastReward: engine.lastReward
      };
      const frameJson = ABREngine._stableStringify(framePayload);
      const encoder = new TextEncoder();
      const frameBytes = encoder.encode(frameJson);
      const frameSection = new ArrayBuffer(4 + frameBytes.length);
      const frameView = new DataView(frameSection);
      frameView.setUint32(0, frameBytes.length, true);
      new Uint8Array(frameSection, 4).set(frameBytes);
      ...
      headView.setUint16(8, sections.length + 1, true); // section count
      sections.unshift(head);
    ...
    decode: function(buffer) {
      ...
      const abrTableOffset = 16;
      for (let i = 0; i < 28; i++) {
        const offset = abrTableOffset + (i * 32);
        blocks.push({
      ...
      const frameOffset = abrTableOffset + (28 * 32);
      let frameStream = null;
      if (frameOffset + 4 <= view.byteLength) {
        const length = view.getUint32(frameOffset, true);
        const frameStart = frameOffset + 4;
        const frameEnd = frameStart + length;
        if (frameEnd <= view.byteLength) {
          const decoder = new TextDecoder();
          const json = decoder.decode(new Uint8Array(view.buffer, frameStart, length));
          frameStream = JSON.parse(json);
        }
      }

      return { version, blocks, frameStream };
    }
  };
