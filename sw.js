/* =====================================================================
   sw.js — lam.o kernel adapter (Ollama PWA)
   - Deterministic API-in / API-out
   - Routes:
   ...
   - No UI logic. No hard-coded models. Model selection is payload-driven.
   ===================================================================== */
...
self.addEventListener('fetch', (e) => {
  ...
  if (url.origin === self.location.origin && url.pathname.startsWith('/api/lam.o/')) {
    e.respondWith(handleLamO(e.request));
    return;
  }
